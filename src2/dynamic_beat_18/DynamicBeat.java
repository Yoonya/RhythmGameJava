...
import dynamic_beat_17.Game;
import dynamic_beat_17.KeyListener;
import dynamic_beat_17.Main;
import dynamic_beat_17.Music;
import dynamic_beat_17.Track;
...
public class DynamicBeat extends JFrame {
	...
	private boolean isMainScreen = false; //메인화면 비활성화(게임고르고있는곳)
	private boolean isGameScreen = false; //게임화면 비활성화(게임들어가서 내부디자인)
	...
	public DynamicBeat() {
		...
		trackList.add(new Track("Soulworker Title Image.png", "Soulworker Start Image.jpg", 
				"Soulworker Game Image.jpg", "Soulworker Selected.mp3", "Soulworker Main Theme Arrange ver..mp3", "Soulworker Main Theme Arrange ver."));
		trackList.add(new Track("Philia Title Image.png", "Philia Start Image.png", 
				"Philia Game Image.jpg", "Philia Selected.mp3", "기적의 분식집 OST - 소금빙수.mp3", "기적의 분식집 OST - 소금빙수"));
		trackList.add(new Track("Fallin Title Image.png", "Fallin Start Image.jpg", 
				"Fallin Game Image.jpg", "Fallin Selected.mp3", "방구석의 인어아가씨 OST - Fall In.mp3", "방구석의 인어아가씨 OST - Fall In"));
		
		setUndecorated(true);//기존 메뉴바 삭제
		setTitle("Soul Beat"); //게임제목
		...
		addKeyListener(new KeyListener()); //KeyListener 클래스 적용
		...
		exitButton.addMouseListener(new MouseAdapter() { //마우스 이벤트 처리(콜백 메소드)
			@Override
	...
	public void paint(Graphics g) {//Jframe에서 약속된것 , Graphics은 콜백메소드로 자동실행
		...
		screenDraw((Graphics2D)screenGraphic); // 스크린 그래픽에 그림을 그려라 screenDraw에서 받아와서 여기서실행, 그래픽2d를 통해 안티애일러싱기능추가
		g.drawImage(screenImage, 0, 0, null); // 그런데 위치를 스크린 이미지를 0, 0 위치에 그려
	}

	public void screenDraw(Graphics2D g) { //그래픽을 입히는부분
		...
		if(isGameScreen) {//게임디자인화면이 true가 됬을때
			game.screenDraw(g);
		}
	...
	public void gameStart(int nowSelected, String difficulty) { //게임시작
		...
		game.start();
		setFocusable(true); //키보드 입력이 제대로 안될 때를 대비해 창에 포커스를 맞추는 것
	}
