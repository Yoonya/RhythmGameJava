...
import dynamic_beat_18.Beat;
import dynamic_beat_18.Main;
import dynamic_beat_18.Music;
import dynamic_beat_18.Note;
...
public class Game extends Thread{ //게임이 실행했을 때에 대하여
	 ...
	 private Image noteRouteLImage = new ImageIcon(Main.class.getResource("../images/noteRoute.png"))
				.getImage();//노트경로
	 private Image blueFlareImage; //점수디자인후광
	 private Image judgeImage; //점수디자인
	 private Image keyPadAImage = new ImageIcon(Main.class.getResource("../images/keyPadBasic.png"))
				.getImage();//키패드디자인
	...
	public void screenDraw(Graphics2D g) {
		...
		for(int i = 0; i< noteList.size(); i++) {//노트배열에서 출력
			...
			if(note.getY() > 940) {
				judgeImage = new ImageIcon(Main.class.getResource("../images/judgeMiss.png")).getImage(); //miss디자인출력
			}
		...
		g.drawString(scoreString, 880, 1053); //점수문장
		g.setColor(Color.yellow); //색깔설정
		g.setFont(new Font("Ariel", Font.BOLD, 80)); //폰트설정
		if(combo != 0)
		g.drawString(comboString, 960, 540); //콤보문장
		g.setColor(Color.LIGHT_GRAY); //색깔설정
		g.setFont(new Font("Elephant", Font.BOLD, 45)); //폰트설정
		g.drawImage(blueFlareImage, 350, 450, null);  
		g.drawImage(judgeImage, 725, 710, null);  
		g.drawImage(keyPadAImage, 360, 870, null);  
		g.drawImage(keyPadSImage, 516, 870, null); 
		g.drawImage(keyPadDImage, 672, 870, null); 
		g.drawImage(keyPadSpace1Image, 828, 870, null); 
		g.drawImage(keyPadSpace2Image, 978, 870, null); 
		g.drawImage(keyPadJImage, 1134, 870, null); 
		g.drawImage(keyPadKImage, 1290, 870, null); 
		g.drawImage(keyPadLImage, 1446, 870, null); 
	}
	...
	public void pressA() { //A를 눌렀을때
		...
		keyPadAImage = new ImageIcon(Main.class.getResource("../images/keyPadPressed.png"))
				.getImage();//키패드가 눌렸을때의 색깔로 바뀌게한다. 
	...
	public void releaseA() { //A를 땠을때
		...
		keyPadAImage = new ImageIcon(Main.class.getResource("../images/keyPadBasic.png"))
				.getImage();
	}
	...
	public void judge(String input) {
		for(int i = 0 ; i< noteList.size(); i++) {
			...
			if(input.equals(note.getNoteType())) { //그것을 노트타입과 비교 A S D .. 비교해서 맞는지
				judgeEvent(note.judge()); //그걸 judge함수로 판단 후 judgeEvent로 출력
				score += note.judgeScore();
				scoreString = Integer.toString(score);
				if(note.judgeCombo() == 1) {
				   combo++;
	...
	public void judgeEvent(String judge) { //판정 디자인 출력
		if(!judge.equals("None")) { // 존재하지 않는 경우 후광 출력
