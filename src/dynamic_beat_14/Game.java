...
public class Game extends Thread{ //게임이 실행했을 때에 대하여
	...
	public void dropNotes() { //떨어지는 노트
		Beat[] beats = null; // 비트 추가
		if(titleName.equals("Soulworker Main Theme Arrange ver.")) { //곡명마다 비트가 다르게
		...
		else if(titleName.equals("기적의 분식집 OST - 소금빙수")) {
			int startTime = 2000 - Main.REACTH_TIME*1000; //이 식을 만듬으로써 항상 판정바에 적중하게 한다.
			beats = new Beat[] {
					new Beat(startTime, "Space"),
			};
		}
		if(titleName.equals("방구석의 인어아가씨 OST - Fall In")) {
